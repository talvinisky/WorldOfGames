...
#     name = lines[0].strip()  # Extract the name from the first line
#     scores = [int(score.strip()) for score in lines[1:] if score.strip().isdigit()]  # Extract scores from subsequent lines
...
#                 # Delete the file after rendering in order for it to work again once a new player starts playing
#                 #os.remove("Scores.txt")
#                 return rendered_template
#             else:
#                 return render_template('Error.html', ERROR="Empty file")
...
#     app.run(host='0.0.0.0', port=8777)  # Changed the port name and make the server  from anywhere
import os
import os
from flask import Flask, render_template
from pathlib import Path
...
def process_scores(lines):
    ...
    scores = [int(score.strip()) for score in lines[1:] if
              score.strip().isdigit()]  # Extract scores from subsequent lines
    total_score = sum(scores)
    return name, total_score
    ...
    try:
        ...
        with open(Path("Scores.txt"), "r") as score_file:
            ...
            if lines:
                ...
                # Uncomment if you want to delete the file after rendering
                # os.remove("Scores.txt")
